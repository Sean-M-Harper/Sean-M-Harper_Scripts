...
for file in $(curl -s $site |
			...
			curl -# -O --progress-bar  $site$file
done

#STEP 2: CONVERT TO SKIP EULA AND INSTALL
...
#STEP 3: SUBMIT INVENTORY TO JSS
echo "SUBMITTING INVENTORY TO THE JSS..."
jamf -recon verbose
echo "FINISHED INVENTORY UPDATE FOR" `hostname -s`
